...
def prepareSlotValuesIndependent(db_path):
    ...
    fin = open(os.path.join(db_path, 'train_db.json'), 'r')
    db_json = json.load(fin)
    fin.close()
...
def delexicalise(utt, dictionary):
    ...
    for key, val in dictionary:
        ...
        utt = re.sub(pattern, val, utt, flags=re.IGNORECASE)
    return utt.strip()


def delexicaliseReferenceNumber(text):
    """based on regex, find the reference number of length 6-8 consisting of digits and uppercase letter and delexicalise it"""
    reference_re = re.compile(r'[A-Z0-9]{8}')
    text = re.sub(reference_re, ' [value_reference] ', text)
    text = re.sub(timepat, ' [value_time] ', text)
    text = re.sub(pricepat, ' [value_price] ', text)
    return text
