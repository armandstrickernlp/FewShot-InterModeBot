...
class MultiWozDB(object,):
    def __init__(self, db_path):
        ...
        self.db_domains = ['restaurant', 'hotel', 'attraction', 'train']
        db_paths = {
            'attraction': f'{db_path}/attraction_db.json',
            'hospital': f'{db_path}/hospital_db.json',
            'hotel': f'{db_path}/hotel_db.json',
            'police': f'{db_path}/police_db.json',
            'restaurant': f'{db_path}/restaurant_db.json',
            'taxi': f'{db_path}/taxi_db.json',
            'train': f'{db_path}/train_db.json',
        }
        for domain in self.all_domains:
            with open(db_paths[domain]) as f:
    ...
    def addBookingPointer(self, turn_da):
        ...
        vector = [0, 0]
        if turn_da.get('Booking-NoBook'): ######################## modified to camel case
            vector = [1, 0]
    ...
    def pointerBack(self, vector, domain):
        ...
        if vector[:4] == [0,0,0,0]:
            ...
            num = vector.index(1)
            report = domain+'='+nummap[num] + ', '  ########### modif
        ...
        for db_ent in self.dbs[domain]:
            ...
            for s, v in constraints.items():
                ...
                if s in ['arriveby', 'leaveat']:
                    ...
                    if s == 'arriveby' and v>time:
                        match = False
                    if s == 'leaveat' and v<time:
                        match = False
