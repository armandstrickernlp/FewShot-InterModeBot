...
from mwzeval.bertscore import BERTScore
...
class Evaluator:
    ...
    def __init__(self, bleu : bool, success : bool, richness : bool, dst : bool = False, jga : bool = False, filter : bool = False, bleu_aug : bool = False):
        self.bleu = bleu
        self.bleu_aug = bleu_aug
        ...
        if bleu_aug:
            with open("data/interference_data/test.json") as f:
                self.interference_data = json.load(f)
    ...
    def evaluate(self, input_data):
        ...
        results = {
            ...
            "interference": get_bleu_augmented(input_data, self.reference_dialogs, self.interference_data) if self.bleu_aug else None,
        ...
        if self.filter:
            final = {
                "bleu": results['semantic']['mwz22-bleu'],
                "bleu_aug": results['interference']['augmented-bleu'] if results['interference'] else None,
                "success": results['task']['success']['total'],
                "inform": results['task']['inform']['total'],
                "jga": results['jga']['jga'],
                "slot_f1": results['dst']['slot_f1'],
                "num_trigrams": results['richness']['num_trigrams'],
            }
            return final
        return results
...
def get_bleu_augmented(input_data, reference_dialogs, interference_data):
    """ Get ScreBleu score only for responses which follow turns augmented with an interference"""

    hyps = []
    refs = []
    
    for dialog_id, dialog in input_data.items():
        backstory_turn_idx = interference_data[dialog_id.upper()]['augmented_idx'][0]
        for turn_idx in range(len(dialog)):
            if turn_idx == backstory_turn_idx // 2: 
                hyps.append(dialog[turn_idx]["response"])
                gold_norm = interference_data[dialog_id.upper()]['log'][backstory_turn_idx+1]['reaction'] \
                    + ' ' \
                    + reference_dialogs['mwz22'][dialog_id][turn_idx] # add original normalized turn to reference chitchat reaction 
                refs.append([gold_norm])

    return {'augmented-bleu': corpus_bleu(hyps, refs).score}
...
def get_jga(input_data, fuzzy_ratio=95):
        ...
        for i, turn in enumerate(input_data[dialog_id]):
            ...
            for domain, gold_domain_state in gold_state.items():
                ...
                for slot, value in gold_domain_state.items():
                    ...
                    if fuzz.partial_ratio(pred_value, value) <= fuzzy_ratio:
                        turn_correct = False
                        break
                if not turn_correct:
                    break
